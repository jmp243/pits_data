...
PITS_ListIncidents_2025 <- read_csv("~/data librarian/PITS_Data/PITS_ListIncidents_Jan2025-April29_2025.csv")
...
PITS_susp <- read_csv("~/data librarian/PITS_Data/PITS_ListSuspensions2023_april29_2025.csv")
...
# 
# # Generate consistent masked IDs
# PITS_2023_2025_public$Location_masked <- generate_consistent_ids(PITS_2023_2025_public$Location_Standard)
# 
# MyPC_public$Location_masked <- generate_consistent_ids(MyPC_public$Location_Standard)
# 
# # Create a lookup table
# lookup_table_branch <- PITS_2023_2025_public %>%
#   select(original_name = Location_Standard, Location_masked) %>%
#   distinct()
...
## SHINY APP for event count and location

```{r}
# PITS Data with Event and Location
# Define UI
ui <- fluidPage(
...
# EXAMPLE of type of parsing task
...
# Convert to ggtexttable and save
p <- ggtexttable(PITS_public_summary, rows = NULL, theme = ttheme("light"))

ggsave("PITS_public_summary_table.png", plot = p, width = 12, height = 8, dpi = 300)
...
PITS_public_summary_table <- PITS_public_summary %>%
  kable("html", 
   ...
   add_header_above(c("PITS Incident Category by Type" = 3)) %>%  # Add table title spanning all 4 columns
  kable_styling(
...
# Define UI
ui <- fluidPage(
  titlePanel("Incident Reports Dashboard"),
  sidebarLayout(
    sidebarPanel(
      filter_slider("Date2", "Date Range", sd2, ~Date2, width = "100%"),
      filter_select("Location_Standard", "Location", sd2, ~Location_Standard, multiple = TRUE),
...
write.csv(hourly_loc_long2, "~/data librarian/PITS_Data/pits_reports_analysis/processed_data/hourly_loc_long2.csv")
write.csv(incident_counts_with_total, "~/data librarian/PITS_Data/pits_reports_analysis/processed_data/incident_counts_with_total.csv")
write.csv(incident_counts, "~/data librarian/PITS_Data/pits_reports_analysis/processed_data/incident_counts.csv")
```