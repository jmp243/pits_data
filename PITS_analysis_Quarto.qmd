...
saveWidget(p, file = "plotly_PITS-date-location-graph.html", selfcontained = TRUE)
...
PITS_public_summary <- PITS_public %>%
  ...
  summarise(count = n(), .groups = "drop") %>%
  mutate(
    percentage = round((count / sum(count)) * 100, 2)  # Calculate & round percentage
  ) %>%
