...
last updated 3/26/2025 \# Navigating Data in Public Libraries: Challenges & Opportunities \## Running Code
...
library(quarto)
# library(rsconnect)
    # rsconnect::deployApp('/Users/jungmeepark/Documents/PCPL/data-analysis/pits_data/Pits_project_mac')
...
# Define UI
ui <- fluidPage(
  titlePanel("PITS with Event and Location"),
  sidebarLayout(
    sidebarPanel(
      filter_slider("Date2", "Date Range", sd, ~Date2, width = "100%"),
      filter_select("Location", "Location", sd, ~Location, multiple = TRUE)
...
PITS_public_summary_table <- PITS_public_summary %>%
  kable("html", 
   ...
   add_header_above(c("PITS Incident Category by Type (Jan 2023 - Mar 2025)" = 3)) %>%  # Add table title spanning all 4 columns
  kable_styling(
