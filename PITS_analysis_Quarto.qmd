...
# Check column names before pivoting
print(colnames(hourly_location_summary))
...
# create a plot for incident type and date filter 
```{r}
library(plotly)
library(dplyr)
library(tidyr)
...
# Create a shared data object
sd <- SharedData$new(incident_counts)
...
filter_panel <- list(
  ...
  filter_select("Incident_Type", "Incident Type", sd, ~Incident_Type, multiple = TRUE)
)
...
# Create a shared data object
sd <- SharedData$new(incident_counts_with_total)
