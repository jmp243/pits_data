...
# # Join back to the original filtered dataframe
# PITS_fixed_categories <- PITS_empty_categories %>%
#   select(-categories) %>%
#   distinct(row_id, .keep_all = TRUE) %>%
#   left_join(category_lookup, by = "row_id") %>%
#   select(-row_id) %>%  # Remove the temporary row_id
#   # Replace the original Infraction Categories with the cleaned version
#   mutate(`Infraction Categories` = clean_categories)

# # To update the main dataframe
# PITS_2023_2025_public_updated <- PITS_2023_2025_public %>%
#   # Mark rows that need updating
#   mutate(needs_update = `Infraction Categories` == "" |
#                         `Infraction Categories` == "null" |
#                         str_detect(`Infraction Categories`, "#") |
#                         is.na(`Infraction Categories`)) %>%
#   # Remove the rows that will be replaced
#   filter(!needs_update) %>%
#   # Remove the temporary column
#   select(-needs_update) %>%
#   # Bind with the fixed rows
#   bind_rows(PITS_fixed_categories)
...
```{r}
# remove source category from PITS_empty to merge
PITS_empty <- PITS_empty_categories %>% 
...
```{r}
# consider three spaces as separators
library(tidyverse)
# 
# # Sample data
# PITS_empty_categories_cleaned <- data.frame(text_column = c(
#   "Medical 911    Called 911 for medical",
#   "Dangerous, Threatening, or Illegal Conduct    1.1.8 Illegal drugs or alcohol: possessing, selling, or using~Disruptive Personal Behavior    2.4 Sleeping~Medical 911    Called 911 for medical",
#   "Disruptive Personal Behavior    2.4 Sleeping~Medical 911",
#   "null    1.1.6 Disorderly conduct, fighting, violence, threatening, intimidating"
# ))

clean_text <- function(text) {
  text %>%
    str_replace_all("\\s{2,}", "|") %>%  # Replace multiple spaces with "|"
    str_remove_all("(?<!\\d)\\d+(\\.\\d+)+") %>%  # Remove numeric codes like 1.1.4 but keep 911
    str_replace_all("\\|+", "|") %>%  # Remove extra delimiters
    str_remove_all("^\\||\\|$")  # Remove leading/trailing separators
}

PITS_empty_categories$categories <- sapply(PITS_empty_categories$category_source, clean_text)
...
clean_text <- function(text) {
  text %>%
    str_replace_all("\\s{2,}", "|") %>%  # Replace multiple spaces with "|"
    str_remove_all("(?<!\\d)\\d+(\\.\\d+)?(?!\\d)") %>%  # Remove numeric codes except 911
    str_replace_all("\\|+", "|") %>%  # Remove extra delimiters
    str_remove_all("^\\||\\|$")  # Remove leading/trailing separators
}

# PITS_empty_categories <- PITS_empty_categories %>%
#   mutate(cleaned_text = sapply(category_source, clean_text)) %>%
#   separate_rows(cleaned_text, sep = "~") %>%  # Split into multiple rows
#   mutate(value = 1) %>%  # Assign a binary indicator
#   pivot_wider(names_from = cleaned_text, values_from = value, values_fill = 0)  # Pivot wider
...
```{r}
# #example with parsed categories
# PITS_empty_categories <- parse_infractions(PITS_empty_categories$category_source)
...
```{r}
# pits summary
PITS_summary <- PITS_categories %>%
  group_by(categories) %>%
  summarise(count = n(), .groups = "drop") %>%
  arrange(categories, desc(count))
