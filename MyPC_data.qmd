...
location_filter <- filter_select("location", "Location", sd4, ~Location_Standard, multiple = TRUE)
date_filter <- filter_slider("date", "Date Range", sd4, ~booking_date, width = "100%")
...
summary_table <- ratio_data %>%
  ...
  # Modify Total_Users to avoid zero division
  mutate(
    # Replace 0 with 0.1 to avoid division by zero
    Total_Users = ifelse(Total_Users == 0, 0.1, Total_Users),
    
    # Calculate Infraction Rate per 1000 users
    Total_Infraction_Rate = (Total_Infractions / Total_Users) * 1000,
    
    # Round numeric columns to 2 decimal places
    Average_Daily_Users = round(Average_Daily_Users, 2),
    Average_Daily_Infractions = round(Average_Daily_Infractions, 2),
    Total_Infraction_Rate = round(Total_Infraction_Rate, 2)
  ) %>%
  # Arrange by Total Infraction Rate in descending order
  arrange(desc(Total_Infraction_Rate))
